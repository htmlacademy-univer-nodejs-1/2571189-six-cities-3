...
describe('DELETE /offer', async () => {
  test('Success delete offer', async (tc) => {
    const user = {
    ...
    let getResponse = await fetch(new URL(`/offers/${offerRdo.id}`, url), {
      method: 'GET',
      headers: new Headers({
        'content-type': 'application/json',
        'Authorization': `Bearer ${content.token}`
    ...
    tc.expect(getResponse.ok).toBeTruthy();
    tc.expect(getResponse.status).toStrictEqual(200);
    ...
    tc.expect(response.status).toStrictEqual(204);

    getResponse = await fetch(new URL(`/offers/${offerRdo.id}`, url), {
      method: 'GET',
      headers: new Headers({
        'content-type': 'application/json',
        'Authorization': `Bearer ${content.token}`
    ...
    tc.expect(getResponse.ok).toBeFalsy();
    tc.expect(getResponse.status).toStrictEqual(404);
  });

  test('Fail delete offer [Not found]', async (tc) => {
    const user = {
      email: 'test@test.com',
      password: '12345678910',
    };
  ...
  test('Fail delete offer [Unauthorized]', async (tc) => {
